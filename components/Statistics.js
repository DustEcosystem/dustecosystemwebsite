...
const Statistics = (props) => {
    ...
    useEffect(() => {
        fetch('https://public-api.solscan.io/token/holders?tokenAddress=DUSTawucrTsGU8hcqRdHDCbuYhCPADMLM2VcCb8VnFnQ&offset=0&limit=10')
            .then(res => res.json())
            .then(data => setDustHoldersCount(data.total))
    },[])

    return (
        <div className="flex flex-col md:flex-row py-5 px-4 sm:px-8 lg:px-12 xl:px-36 justify-around items-center gap-4">
            <div className="flex flex-col p-3 sm:p-6 text-center justify-center w-full bg-[#242424] rounded-xl gap-2">
                <div className="text-xl text-white fontFamily">
                    DUST Market Cap
                </div>
